...
| `/res set admin 所有者` | 只有所有者可以执行所有领地管理操作 |
| `/res set admin.trust 成员` | 成员及以上可以信任或取消信任玩家 |
...
| `dropitem.throw`      | 丢弃物品      |
| `dropitem.pickup`     | 拾取物品      |
| `use`                 | 使用物品      |
| `fly`                 | 鞘翅飞行      |
| `admin`               | 所有领地管理操作  |
| `admin.trust`         | 信任或取消信任玩家 |
| `admin.setpermission` | 修改领地权限规则  |
| `admin.removeclaim`   | 删除领地      |
| `admin.subclaim`      | 创建子领地     |
| `admin.area.add`      | 添加领地区域    |
| `admin.area.remove`   | 删除领地区域    |
| `admin.area.list`     | 查看领地区域列表  |
...
表示所有 `block` 下级权限，例如破坏、放置、交互方块。

例如检查 `admin.area.add` 时，系统会按下面的顺序寻找最具体的规则：

```text
admin.area.add -> admin.area -> admin -> *
```
